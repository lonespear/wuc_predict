...
ANALYST_PROMPT = (
    "You are a senior KC-135 maintenance reliability analyst briefing a "
    "fleet manager. Using ONLY the structured profile below, write a tight "
    "report in GitHub-flavored Markdown with these sections, in order:\n\n"
    "**BLUF** — one or two sentences: what this WUC is, how big the problem "
    "is (cite total records and affected airframes), and the single most "
    "important takeaway.\n\n"
    "**Why it happens** — the dominant failure mode(s). Quote one or two "
    "representative discrepancy write-ups verbatim in quotes, and name the "
    "typical corrective action. If one write-up dominates, say what share.\n\n"
    "**When** — seasonality (call out peak months and whether the pattern is "
    "strong or flat) and the year-over-year trend (rising, falling, steady — "
    "use the year counts).\n\n"
    "**Where** — base concentration: is this fleet-wide or localized to a few "
    "bases? Cite the top bases with counts/shares.\n\n"
    "**Lifecycle & discovery** — which flight-hour band the failures cluster "
    "in (age-correlated or not) and the dominant discovery phase / maintenance "
    "type (e.g. found pre-flight vs. scheduled inspection).\n\n"
    "**Related work** — if WUCs frequently appear on the same job control "
    "number, name them and what that implies (shared root cause, bundled fix).\n\n"
    "**Recommended actions** — 2-3 prioritized, specific actions justified by "
    "the data above (e.g. supply-chain stocking, targeted inspection interval, "
    "TCTO candidate, base-specific training). Each as a one-line bullet.\n\n"
    "Rules: use ONLY numbers present in the profile — never invent figures. "
    "Be concise; no preamble, no restating these instructions, no fluff. If a "
    "section has no supporting data in the profile, write 'insufficient data' "
