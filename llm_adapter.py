...
ANALYST_PROMPT = (
    ...
    "manager. The PROFILE below is a JSON object. Read its fields and write a "
    "tight report in GitHub-flavored Markdown with these sections, in order. "
    "Each section names the JSON field(s) you must summarize — if that field "
    "has entries, you MUST use them; only write 'insufficient data' for a "
    "section when its source field is literally empty ({}, [], or null).\n\n"
    "**BLUF** — 1-2 sentences: what this WUC is (`wuc`, `description`), the "
    "scale (`total_records`, `affected_tails`, `date_range`), and the single "
    "biggest takeaway.\n\n"
    "**Why it happens** — from `top_discrepancy_phrases` and "
    "`top_discrepancy_keywords`: the dominant failure mode; quote one or two "
    "write-ups verbatim with their counts, and if one dominates give its share "
    "of `total_records`. Name the typical fix from `top_corrective_actions`.\n\n"
    "**When** — seasonality from `month_histogram` (peak months + whether the "
    "pattern is pronounced or flat) and the year-over-year trend from "
    "`year_histogram` (compare the earliest and latest years: rising, falling, "
    "or steady — give the numbers).\n\n"
    "**Where** — from `base_distribution` ({base: count}): name the top 2-3 "
    "bases with counts, and state whether the problem is fleet-wide or "
    "concentrated at a few bases (e.g. if the top base is a large share of "
    "`total_records`).\n\n"
    "**Lifecycle & discovery** — from `flight_hour_buckets` (four airframe "
    "age quartiles): are failures roughly even across age or skewed to "
    "high/low hours? And from `when_discovered_phase` / `maint_type_phase`: "
    "the dominant way these are caught (e.g. scheduled inspection vs. servicing "
    "vs. in-flight).\n\n"
    "**Related work** — from `cooccurring_wucs` ({wuc: count} on the same job "
    "control number): name them and what it implies (shared root cause, bundled "
    "fix, common parent assembly).\n\n"
    "**Recommended actions** — 2-3 prioritized one-line bullets, each justified "
    "by a specific number from the profile above (e.g. supply-chain stocking, a "
    "targeted inspection interval tied to the peak months or age band, a TCTO "
    "candidate, base-specific attention).\n\n"
    "Rules: use ONLY numbers present in the profile — never invent figures. Be "
    "concise; no preamble, no restating these instructions, no closing remarks.\n\n"
    "PROFILE:\n"
)
